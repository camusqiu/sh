...
#grep "title" ../temp/urlnametimeTemp.txt>$urlname
...
#sed -e 's///g' $urlname|awk -F 'html">' 'NF>1{print $2}' |awk -F ' alt' 'NF>1{printf $1}'|sed -e 's/[&amp;amp;quot]//g'|sed -e 's/"/\n/g'>$name

#time
#rm -rf $time
#tag=0
#awk -F '<h3>' 'NF>1{print $2}' $urlname|awk -F '<' 'NF>1{printf $1}'|sed -e 's/[^ -z]//g'>$timeTemp
#strvalue=$(cat $timeTemp)
#len=$(echo -n $strvalue|wc -c)
#while [[ $len -ge 16 ]]
#do
#    echo $len
#    echo ${strvalue:${tag}:16}>>$time
#    let len=len-16
#    let tag=tag*16
#done
